...
let equations = inputTest.split(separator: "\n").map { $0.split(separator: ":") }

let result1 = await calculateEquations(input: equations, operators: ["+", "*"])
let result2 = await calculateEquations(input: equations, operators: ["+", "*", "|"])

print("\ntime: \(Date.now.timeIntervalSince(date))")
print("sum: \(result1)")
print("sum: \(result2)")

func calculateEquations(input: [[Substring.SubSequence]], operators: [String]) async -> Int {
...
func getRightValue(_ equation: [Substring.SubSequence], operators: [String]) async -> Int {
