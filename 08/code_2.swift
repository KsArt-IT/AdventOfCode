...
func findAntinodes(_ input: [[Character]], all: Bool = false) -> Set<Position> {
    ...
    return if !all {
        ...
        findAntinodesPart2(map, width: width, height: height)
    }
}
...
func findAntinodesPart2(_ map: [Character: [Position]], width: Int, height: Int) -> Set<Position> {
    var antinodes = Set<Position>()

    func addAntinode(_ n: Position,_ step: Int, _ dx: Int,_ dy: Int) {
        let x = n.x + step * dx
        let y = n.y + step * dy
        if 0..<width ~= x, 0..<height ~= y {
            antinodes.insert(Position(x: x, y: y))
    ...
    for (_, positions) in map {
        for i in 0..<positions.count {
            for j in i + 1..<positions.count {
                ...
                for step in 1...max(width, height) {
                    addAntinode(p1, -step, dx, dy)
                    addAntinode(p2, step, dx, dy)
                }
            }
        }
...
let result2 = findAntinodes(map, all: true)
print("2: \(result2.count)")
