...
func isReportSafe(_ line: String) -> Bool {
    ...
    guard !checkReportSafe(numbers) else { return true }

    // bad report check
    for level in 0..<numbers.endIndex {
        var badReport = numbers
        badReport.remove(at: level)
        if checkReportSafe(badReport) {
            return true
        }
    }
    return false
...
func checkReportSafe(_ numbers: [Int]) -> Bool {
