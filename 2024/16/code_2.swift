...
if let start, let end {
    let (result, result2) = findPath(on: map, start: start, end: end)
    print(result)
    print(result2)
} else {
...
// MARK: - Func
func findPath(on map: [[Character]], start: Point, end: Point) -> (Int, Int) {
    var queue: [State] = [State(position: start, direction: .east, cost: 0, path: [start])]
    var paths = [Int: [[Point]]]()
    ...
    while let state = queue.popLast()  {
        ...
        if state.position == end {
            ...
            if state.cost == minCost {
                paths[minCost, default: []].append(state.path)
            }
            continue
        }
    ...
    var points = 0
    if let pathsAtMinCost = paths[minCost], !pathsAtMinCost.isEmpty {
        points = Set(pathsAtMinCost.flatMap(\.self)).count
    }

    return (minCost, points)
}
